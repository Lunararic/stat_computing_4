...
// [[Rcpp::export]]
arma::colvec  admm_lasso_elastic_net(arma::mat X, arma::colvec y, double tau, int maxit, double tol) {
	arma::mat XX = X.t() * X;
	arma::mat Xy = X.t() * y;
	int p = X.n_cols;
	double alpha = 0.95;
	...
	arma::mat Sinv = arma::inv( XX + arma::eye(p, p) * rho + (1-alpha) * tau * arma::eye(p, p) );
	for (int i = 1; i <= maxit; i++ ) {
		beta = Sinv * (Xy + rho * z - lambda);
		z = soft_thresh(beta + lambda * (1/rho), alpha * tau * (1/rho) );
